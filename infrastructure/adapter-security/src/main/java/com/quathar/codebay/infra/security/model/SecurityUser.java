...
public final class SecurityUser implements UserDetails {
    ...
    // <<-CONSTANT->>
    /**
     * Represents the prefix used for security roles.
     */
    ...
    public java.util.Collection<? extends GrantedAuthority> getAuthorities() {
        var grantedAuthorities = this.role
                .getGrantedPermissions()
                .map(GrantedPermission::getOperation)
                .map(Operation::getName)
                .map(SimpleGrantedAuthority::new)
                .collect(java.util.stream.Collectors.toSet());
        var roleAuthority = new SimpleGrantedAuthority( ROLE_PREFIX + this.role.getName() );
