...
public interface JpaProductRepository extends JpaRepository<ProductEntity, java.util.UUID> {
    ...
    @Query("""
        ...
        WHERE
            ...
            AND (:#{#filters.maxStock}     IS NULL OR p.stock     <= :#{#filters.maxStock})
            AND (:#{#filters.minUnitsSold} IS NULL OR p.unitsSold >= :#{#filters.minUnitsSold})
            AND (:#{#filters.maxUnitsSold} IS NULL OR p.unitsSold <= :#{#filters.maxUnitsSold})
            AND (:#{#filters.minDiscount}  IS NULL OR p.discount  >= :#{#filters.minDiscount})
            AND (:#{#filters.maxDiscount}  IS NULL OR p.discount  <= :#{#filters.maxDiscount})
    """)
